...
@app.route("/play")
def play():
    return render_template("TandonGeoguessrPlay.html")
...
def signin():
    ...
    if request.method == "POST":
        ...
        db = database()
        cursor = db.cursor(dictionary=True)
        cursor.execute("""SELECT player_id, username, password FROM player where email=%s and password =%s""",(email,password))
        user = cursor.fetchone()
        cursor.close()
        db.close()
        if user:

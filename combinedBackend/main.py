...
def submit_guess():
    ...
    session_id_for_guess = session.get("current_session_id")
    if session_id_for_guess is None and mode == "normal":
...
def result():
    ...
    if mode == "normal" and is_final_round:
        weekStart, weekEnd = currentWeek()
        db2 = database()
        cur2 = db2.cursor(dictionary=True)
        # get this week's leaderboard row
        cur2.execute(
            """SELECT leaderboard_id FROM leaderboard WHERE start_date = %s AND end_date = %s """,(weekStart, weekEnd),)
        lb = cur2.fetchone()
        if not lb:
            cur2.execute("""INSERT INTO leaderboard (start_date, end_date)VALUES (%s, %s)""",(weekStart, weekEnd),)
            leaderboard_id = cur2.lastrowid
        else:
            leaderboard_id = lb["leaderboard_id"]

        # check existing entry
        cur2.execute("""SELECT max_score FROM leaderboard_entry WHERE player_id = %s AND leaderboard_id = %s""",(user["player_id"], leaderboard_id),)
        row = cur2.fetchone()
        if row is None:
            # first score
            cur2.execute("""INSERT INTO leaderboard_entry(leaderboard_id, player_id, max_score, ranking)VALUES (%s, %s, %s, %s)""",(leaderboard_id, user["player_id"], total_score, 0),)
        elif total_score > row["max_score"]:
            # new personal best
            cur2.execute(""" UPDATE leaderboard_entry SET max_score = %s WHERE player_id = %s AND leaderboard_id = %s""",(total_score, user["player_id"], leaderboard_id),)
        db2.commit()
        cur2.close()
        db2.close()

...
def leaderboard():
    ...
    cursor.close()
    db.close()
    return render_template("TandonGeoguessrLeaderboard.html", username = user["username"], best_score = user["best_score"], data=rows, date1=selected_week["start_date"] if selected_week else None, date2=selected_week["end_date"] if selected_week else None, weeks = weeks, selected_week = selectedID)
